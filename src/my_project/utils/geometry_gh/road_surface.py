...
def get_left_vectors(tangent: rg.Vector3d) -> Vector2D:
    ...
    tx = tangent.X / length
    ty = tangent.Y / length
    return Vector2D(x=-ty, y=tx)


def get_points_along_arc(
    start_point: rg.Point3d,
    end_point: rg.Point3d,
    type_info: typeInfo,
) -> tuple[list[rg.Point3d], list[float], list[rg.Vector3d], rg.Vector3d]:
...
def make_clothoid_from_endpoints_radii(
    ...
    dk = k1 - k0
    if abs(dk) < 1e-12:
        raise ValueError("R0 and R1 are equal; clothoid cannot be created.")
    L = end_STA - start_STA
    ...
    world_pts = [
        transform_local_points_with_p0_and_angle(local_pt, p0, rot_angle)
...
def get_indiv_center_line_points(
    ...
    plan_key_points = [const_point_obj(p) for p in road_center_info.plan_Coord_infos]
    ...
    for tangent in center_line_tangents_2D:
        left_vectors.append(get_left_vectors(tangent))
